        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8">
            ...
            {/* ... (Footer etc) ... */}
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <div className="bg-slate-800/50 p-6 rounded-3xl text-center border border-slate-700">
                          <p className="text-xs text-slate-500 mb-2">上位10% (豪運)</p>
                          <p className="text-3xl font-black text-emerald-400">{result.top10} <span className="text-sm">日</span></p>
                        </div>
                        <div className="bg-blue-600/20 p-8 rounded-3xl text-center border-2 border-blue-500 shadow-2xl shadow-blue-500/20 transform scale-110">
                          <p className="text-xs text-blue-300 mb-2">中央値 (平均)</p>
                          <p className="text-5xl font-black text-white">{result.median} <span className="text-xl">日</span></p>
                        </div>
                        <div className="bg-slate-800/50 p-6 rounded-3xl text-center border border-slate-700">
                          <p className="text-xs text-slate-500 mb-2">下位10% (不運)</p>
                          <p className="text-3xl font-black text-red-400">{result.bottom10} <span className="text-sm">日</span></p>
                        </div>
                      </div>
                      <p className="text-center text-sm text-slate-500 mt-8">※ 1日 {staminaPerDay} {gameId === "genshin" ? "樹脂" : "スタミナ"} 消費想定</p>
                      
                      {recycleComparison && (
                        <div className="mt-8 pt-8 border-t border-slate-700">
                          <h4 className="text-center text-lg font-bold mb-4">廻聖（リサイクル）効率比較</h4>
                          <div className="grid grid-cols-2 gap-4">
                            <div className="bg-slate-800/50 p-6 rounded-3xl text-center border border-slate-700">
                              <p className="text-xs text-slate-500 mb-2">リサイクルなし</p>
                              <p className="text-3xl font-black text-slate-300">{recycleComparison.withoutRecycle} <span className="text-sm">日</span></p>
                            </div>
                            <div className="bg-emerald-600/20 p-6 rounded-3xl text-center border border-emerald-500">
                              <p className="text-xs text-emerald-400 mb-2">リサイクルあり</p>
                              <p className="text-3xl font-black text-emerald-400">{recycleComparison.withRecycle} <span className="text-sm">日</span></p>
                            </div>
                          </div>
                          <div className="mt-4 bg-blue-600/10 p-4 rounded-2xl border border-blue-500/30 text-center">
                            <p className="text-sm text-blue-300 font-bold">リサイクルで <span className="text-lg text-blue-400">{recycleComparison.daysSaved.toFixed(1)}</span> 日短縮できます！</p>
                            <p className="text-xs text-slate-400 mt-1">（スタミナ {recycleComparison.staminaSaved.toFixed(0)} 節約）</p>
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  {result.type === "period" && (
                    <div className="space-y-6">
                      <h3 className="text-center text-xl font-bold">{days}日間の厳選で期待できるスコア</h3>
                      <div className="grid grid-cols-3 gap-4">
                        <div className="bg-slate-800/50 p-6 rounded-3xl text-center border border-slate-700">
                          <p className="text-xs text-slate-500 mb-2">下位10%</p>
                          <p className="text-3xl font-black text-slate-400">{result.bottom10.toFixed(1)}</p>
                        </div>
                        <div className="bg-blue-600/20 p-8 rounded-3xl text-center border-2 border-blue-500 shadow-2xl shadow-blue-500/20 transform scale-110">
                          <p className="text-xs text-blue-300 mb-2">中央値 (平均)</p>
                          <p className="text-5xl font-black text-white">{result.median.toFixed(1)}</p>
                        </div>
                        <div className="bg-slate-800/50 p-6 rounded-3xl text-center border border-slate-700">
                          <p className="text-xs text-slate-500 mb-2">上位10%</p>
                          <p className="text-3xl font-black text-emerald-400">{result.top10.toFixed(1)}</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {result.type === "rank" && (
                    <div className="flex flex-col items-center gap-6">
                      <div ref={cardRef} className="bg-slate-900 border border-slate-700 rounded-[40px] p-10 text-center w-full max-w-[480px] shadow-2xl relative overflow-hidden">
                        <div className={`absolute top-0 left-0 w-full h-2 bg-gradient-to-r ${config.gradient}`}></div>
                        <p className="text-sm font-bold text-slate-500 mb-4">{config.name} {config.equipName}ランク診断</p>
                        <h2 className="text-xl font-black text-white mb-8">{characterName}</h2>
                        
                        <p className="text-xs text-slate-500 mb-1">現在の合計スコア</p>
                        <p className="text-6xl font-black text-white mb-6 tracking-tighter">{result.score.toFixed(1)}</p>
                        
                        <div className="text-4xl mb-4 tracking-[0.2em] text-yellow-500">
                          {"★".repeat(result.stars)}{"☆".repeat(5-result.stars)}
                        </div>
                        
                        <p className="text-xl font-bold text-blue-400 mb-8">
                          上位 <span className="text-4xl">{(100 - result.percentile).toFixed(1)}</span> %
                        </p>
                        
                        <div className="bg-slate-800/50 rounded-3xl p-6 text-sm text-slate-300 leading-relaxed border border-slate-700/50">
                          {result.percentile >= 90 ? "神の領域です。これ以上の厳選は宇宙の終わりを待つようなものです。" :
                           result.percentile >= 70 ? "非常に優秀です！実戦では無双できるレベルに達しています。" :
                           result.percentile >= 50 ? "平均より上ですが、伸びしろがあります。もう一息頑張りましょう！" :
                           "まだまだ修行が必要です。まずはメインステータスを揃えるところから！"}
                        </div>
                      </div>
                      <button onClick={downloadImage} className="bg-slate-800 hover:bg-slate-700 text-white px-8 py-3 rounded-2xl font-bold flex items-center gap-2 transition-all">
                        📸 結果を保存する
                      </button>
                    </div>
                  )}
                  {/* Damage Index Display */}
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-700 opacity-80 hover:opacity-100 transition-opacity">
                ...
                <div className="bg-slate-800/30 backdrop-blur-xl border border-slate-700/30 rounded-3xl p-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {Object.entries(compareResult.pieces || {}).map(([slot, art]: [string, any]) => (
