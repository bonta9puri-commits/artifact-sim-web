...
export default function NicoleSpecialPage() {
  ...
  const [scoreWeights, setScoreWeights] = useState(character.defaults?.weights || { [STAT_IDS.CRIT_RATE]: 2.0, [STAT_IDS.CRIT_DMG]: 1.0, [STAT_IDS.ATK_PER]: 1.0 });
  const [mainStats, setMainStats] = useState(character.defaults?.mainStats || { "時の砂": STAT_IDS.ATK_PER, "空の杯": STAT_IDS.PYRO_DMG, "理の冠": STAT_IDS.CRIT_RATE });
                ...
                {simMode === "target" && (
                   <div className="animate-in fade-in slide-in-from-top-2 duration-300 space-y-2">
                     ...
                     <div className="flex flex-wrap gap-1.5">
                       {[360, 390, 420, 450, 480].map(val => (
                         <button
                           key={val}
                           type="button"
                           onClick={() => setTargetScore(val)}
                           className={`px-2.5 py-1 rounded-lg text-[10px] font-black transition-all border ${targetScore === val ? 'bg-orange-600 border-orange-500 text-white shadow-md shadow-orange-500/20' : 'bg-slate-950 border-white/5 text-slate-400 hover:text-white hover:bg-slate-800'}`}
                         >
                           {val}
                         </button>
                       ))}
